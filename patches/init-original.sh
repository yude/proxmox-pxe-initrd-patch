...
if [ -n "$lvm2root" ]; then
    ...
    /bin/ip addr add 127.0.0.1/8 dev lo 2>/dev/null
    ...
    if [ -z "$NET_IF" ]; then
        ...
        /bin/ip link set "$NET_IF" up 2>/dev/null

        # Wait for link to come up
        sleep 2

        # Try DHCP first (quick timeout - may not have DHCP server)
        if [ -x /sbin/udhcpc ]; then
            /bin/echo "Requesting DHCP on $NET_IF..."
            /sbin/udhcpc -i "$NET_IF" -n -q -t 3 2>/dev/null
        fi

        # If DHCP failed, configure static IP
        if ! /bin/ip addr show "$NET_IF" | grep -q "inet "; then
            /bin/echo "DHCP unavailable, configuring static IP..."
            /bin/ip addr add 192.168.100.18/24 dev "$NET_IF"
            /bin/ip route add default via 192.168.100.1 dev "$NET_IF"
            /bin/ip link set "$NET_IF" up
            sleep 2
            /bin/echo "Static IP configured: 192.168.100.18/24 gw 192.168.100.1"
        fi

        if /bin/ip addr show "$NET_IF" | grep -q "inet "; then
            /bin/echo "Network interface $NET_IF is up: $(/bin/ip addr show "$NET_IF" | grep 'inet ')"
